...
import com.nick.mowen.wearossms.task.AvatarLoaderTask
...
fun ImageView.setProfilePhoto(cache: LruCache<String, Bitmap>, path: String? = PreferenceManager.getDefaultSharedPreferences(context)?.getString("avatar", ""), size: Int = 50) {
    if (size == 50) {
        val cached = cache.get("avatar")
    ...
    AvatarLoaderTask(cache, this, size).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, path
            ?: "")
}
...
private fun Context.openScaledPhoto(address: String): Bitmap? {
    ...
    try {
        if (phones.moveToFirst()) {
            ...
            if (path != null)
                return Bitmap.createScaledBitmap(MediaStore.Images.Media.getBitmap(contentResolver, Uri.parse(path)).getCircleBitmap(), 50, 50, true)
        }
...
private fun Context.openSmallPhoto(address: String): Bitmap? {
    ...
    try {
        if (phones.moveToFirst()) {
            ...
            if (path != null)
                return MediaStore.Images.Media.getBitmap(contentResolver, Uri.parse(path)).getSmallCircleBitmap()
        }
    } catch (ignored: Exception) {
    } finally {
        phones.close()
...
private fun String.getLetters(): String {
    return if (length > 0) {
        if (contains(" ")) {
            val section = split(" ".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
            section[0][0] + "" + section[1][0]
        } else {
            val one = this[0]
            if (Character.isLetter(one)) (one + "" + this[1]).toUpperCase() else "#"
        }
    } else
        "#"
}

fun Context.loadBitmapFromAsset(asset: Asset?): Bitmap? {
    if (asset == null)
        throw IllegalArgumentException("Asset must be non-null")

    val inputStream = Tasks.await(Wearable.getDataClient(this).getFdForAsset(asset)).inputStream
